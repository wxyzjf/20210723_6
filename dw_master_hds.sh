[eloisewu@bigdataetl01 script]$ cat dw_master_hds.sh
#! /bin/ksh
#
...
function errhandle {
  MVmsg="$1"
        ...
        maillist="DWSupport@smartone.com 64614596@mash.smartone.com 91349609@mash.smartone.com 92423863@mash.smartone.com 96892473@mash.smartone.com 98645372@mash.smartone.com 98645438@mash.smartone.com 57296942@mash.smartone.com GZ-ISSupport@smartone.com"
        ...
        do
                ...
                do
                        ...
                        if [ $filecnt -eq 0 ]; then
                                ...
                                do
                                        echo "Mv File $f ${MVdestdir}/proc_${fd}"|tee -a $LOGFILE
...
#dispatch_file_head ${PROC_NUM} $INPUTDIR8 $CTRDPHDIR/process "*.csv" 100000
...
CheckFileCnt "${INPUTDIR}/CTR_EV*" "*csv" "filecnt"
function CheckFileCnt {
        ...
        eval "${MVfilenum}=${res}"
        echo "Check file count = ${res}" | tee -a $LOGFILE
        if [ $res -eq 0 ]; then
...
function BackupFile {
        ...
        if [ $? -ne 0 ];then
                echo "Backup File $bkgFile to ${bkgDir}/${bkgTarFile} Error"|tee -a $LOGFILE
                errhandle "Backup File $bkgFile to ${bkgDir}/${bkgTarFile} Error"|tee -a $LOGFILE
                exit 1
        fi
}
